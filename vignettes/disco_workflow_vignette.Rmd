...
`discoGetSimu()` to access the CSV system file:
...
CSV inputs to the DiscoRhythm web interface can be read into R as
a data.frame in this format. It then may be used to construct a 
`r BiocStyle::Biocpkg("SummarizedExperiment")` which will contain:

1) rownames(se) - The feature IDs found in the first column of the data.frame 
will be stored here.
2) assay(se) - A matrix containing experimental data from the subsequent 
columns of the input data.frame.
3) colData(se) - Stores sample metadata which may be extracted from the column 
names of the input data.frame using `discoParseMeta` using the naming scheme 
described in section \@ref(column-naming) or can be generated by other means. 
...
SummarizedExperiemnt objects generated by other packages (as is often the case
with other Bioconductor packages) should be suitable inputs to the below
workflow once modified to contain the metadata in (1) and (3).

### discoDFtoSE

To allow for users using the web application to use the same input for
analysis in R, a function is available to convert the tabular input into a
more apporpriate format for analysis in R. The sample metadata will be 
...
`discoSEtoDF` is also available, however, it is recommended to only use this
...
Below the results of the outlier detection analysis are used to subset the
...
OVpcaSE <- discoDFtoSE(data.frame("PC"=1:ncol(OVpca$x),t(OVpca$x)),
                                  colData(FinalSE))
knitr::kable(discoODAs(OVpcaSE)$CS, format = "markdown")
```
...
which includes the 
visualizations seen in the DiscoRhythm application. `indata` may be a
data.frame (will be automatically converted to `SummarizedExperiment`) or
