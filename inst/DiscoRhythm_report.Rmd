...
```{r }
plotly::ggplotly(plotAvgCor(Metadata,CorRes$meanCor,
...
gridExtra::grid.arrange(
  plotPCAWithShape(PCAres$x,Metadata,col = "None",1,2,PCAres$outliers)+
    ggtitle("Before Outlier Removal"),
  plotPCAWithShape(PCAresAfter$x,Metadata,col = "None",1,2)+
...
```{r }
plotOVpcaScatter(OVpca = OVpca,regressionMeta,osc_period)
```
...
```{r }
knitr::kable(
  discoODAs(data.frame("PC"=1:ncol(OVpca$x),t(OVpca$x)),regressionMeta)$CS,
  format = "markdown"
)
```
...
```{r }
plotPQValueHist(discoODAres,bg = seq_len(nrow(regressionData)),50,
...
plotAmpliHist(discoODAres,bg = seq_len(nrow(regressionData)),
              fg=fg[['CS']],50,'')
```
