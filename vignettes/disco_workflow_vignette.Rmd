...
knitr::kable(head(discoParseMeta(colnames(indata)[-1])), format = "markdown")
```
...
`discoGetSimu()` to access the system file:
...
The two complementary data objects used frequently in the DiscoRhythm R
package are:

1) Maindata - A data.frame following the format above (described in section 
\@ref(input-datasets)). This contains the observed values of the experiment.
2) Metadata - A data.frame with columns for all metadata relating to the 
samples. This 
object may be generated by the `discoParseMeta` function extracting from the 
column names of the first data.frame (using the naming scheme described in 
section \@ref(column-naming)) or can be generated by other means. 3 columns 
are expected:
...
Maindata and Metadata may be conveniently combined into a single MaindataSE `r BiocStyle::Biocpkg("SummarizedExperiment")` object for use as an input throughout the workflow. Maindata arguments to the DiscoRhythm functions below could be substituted with MaindataSE in all following code execution.
...
Extracts all sample metadata necessary for analysis using a regular 
expression[^4] matching the format described in section \@ref(column-naming). 
It also performs the column checks (validity and uniqueness) mentioned in 
section \@ref(select-data). 
...
The sample collection information present in `Metadata` can be summarized by 
the `discoDesignSummary` function to detail the number of biological and technical
replicates available at each collection time. Number of technical replicates is 
shown in brackets.
...
### discoDFtoSE/discoSEtoDF

For interoperability with other Bioconductor packages, it may be desirable to import data as a SummarizedExperiment object for use in place of the `Maindata` plain tabular format.

Convenient functions have been provided in DiscoRhythm for converting to and from the SummarizedExperiment format:

```{r}
### Two methods of constructing a SummarizedExperiment
# 1) Directly from Maindata
# Performs discoParseMeta to get colData
MaindataSE <- discoDFtoSE(Maindata)
# 2) Merge the generated Maindata and Metadata into one
MaindataSE <- discoDFtoSE(Maindata,Metadata)
Maindata <- discoSEtoDF(MaindataSE)
```
...
Performs the analysis described in \@ref(inter-sample-correlation).

```{r}
CorRes <- discoInterCorOutliers(Maindata,
                                cor_method="pearson",
                                threshold=3,
                                thresh_type="SD")
```
...
Performs the analysis described in \@ref(principal-component-analysis).

```{r}
PCAres <- discoPCAoutliers(Maindata,
                           threshold=3,
                           scale=TRUE,
...
Below the results of the outlier detection analysis are shown:

```{r}
Maindata_filtered <- data.frame(Maindata[,1],
                        Maindata[,-1][,!PCAres$outliers & !CorRes$outliers])
Metadata_filtered <- Metadata[!PCAres$outliers & !CorRes$outliers,]

DT::datatable(Metadata[PCAres$outliers | CorRes$outliers,])

knitr::kable(discoDesignSummary(Metadata_filtered),format = "markdown")
```
...
Performs the analysis described in \@ref(row-selection).

```{r}
ANOVAres <- discoRepAnalysis(Maindata_filtered,
                             Metadata_filtered,
                             aov_method="Equal Variance",
                             aov_pcut=0.05,
                             aov_Fcut=1,
                             avg_method="Median")
Maindata_combined <- ANOVAres$regDat
Metadata_combined <- ANOVAres$regMet
```
...
Performs the analysis described in \@ref(period-detection).

```{r}
PeriodRes <- discoPeriodDetection(Maindata_combined,
                                  Metadata_combined,
                                  timeType="linear",
                                  main_per=24)
```
...
```{r}
OVpca <- discoPCA(Maindata_combined)
knitr::kable(discoODAs(data.frame("PC"=1:ncol(OVpca$x),t(OVpca$x)),
                       Metadata_combined)$CS, format = "markdown")
```
...
Performs the analysis described in \@ref(oscillation-detection) using just the
Cosinor method.

```{r}
discoODAres <- discoODAs(Maindata_combined,
                         Metadata_combined,
                         period=24,
                         method="CS",
                         ncores=1,
...
called through a single call to `discoBatch()`.
...
which includes the 
visualizations seen in the DiscoRhythm application.
