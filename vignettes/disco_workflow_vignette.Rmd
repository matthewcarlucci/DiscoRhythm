...
Examples of valid DiscoRhythm naming styles:
...
For each of the sections below, refer to the DiscoRhythm R package manual for 
more details on function usage, arguments and methods or use `?`. For instance, 
get more 
...
The two complementary data objects used frequently in the DiscoRhythm R package
are:
...
knitr::kable(discoDesignSummary(MetaFinal),format = "markdown")
```
...
```{r}
ANOVAres <- discoRepAnalysis(DataFinal,
                             MetaFinal,
...
knitr::kable(discoODAs(data.frame("PC"=1:ncol(OVpca$x),t(OVpca$x)),
                       regressionMeta)$CS, format = "markdown")
```
